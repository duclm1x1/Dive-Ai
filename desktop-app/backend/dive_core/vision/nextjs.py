...
class NextJsPlugin:
    ...
    def suggested_gates(self, repo_root: str) -> List[GateSpec]:
        ...
        out: List[GateSpec] = []
        if 'lint' in sc:
            out.append(GateSpec(name='next-lint', cmd=run_script_cmd(pm, 'lint')))
        if 'typecheck' in sc:
            out.append(GateSpec(name='next-typecheck', cmd=run_script_cmd(pm, 'typecheck')))
        if 'test' in sc:
            out.append(GateSpec(name='next-test', cmd=run_script_cmd(pm, 'test')))
        if 'build' in sc:
            out.append(GateSpec(name='next-build', cmd=run_script_cmd(pm, 'build')))
        return out
