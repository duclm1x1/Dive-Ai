"""
Dive-Memory v3: Core Memory System
Persistent memory with semantic search, knowledge graph, and cloud sync
"""
...
class Memory:
    ...
    relationship: Optional[str] = None  # For related memories
    strength: float = 0.0  # Relationship strength


class DiveMemory:
    """Main Dive-Memory v3 class"""
    ...
    def add(self, content: str, section: str, subsection: Optional[str] = None,
        ...
        cursor.execute("""
            INSERT INTO memories (id, content, section, subsection, embedding, tags, 
                                importance, metadata, created_at, updated_at, access_count)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
        """, (
        ...
        self._auto_link_memory(memory_id)
    ...
    def get_related(self, memory_id: str, max_depth: int = 2) -> List[Memory]:
        ...
        for row in cursor.fetchall():
            memory = self._row_to_memory(row[:12])  # First 12 columns are memory fields
            memory.relationship = row[12]  # relationship
            memory.strength = row[13]  # strength
            results.append(memory)
    ...
    def get_stats(self, section: Optional[str] = None) -> Dict[str, Any]:
        ...
        if section:
            cursor.execute("SELECT COUNT(*), AVG(importance), SUM(access_count) FROM memories WHERE section = ?", (section,))
        else:
            cursor.execute("SELECT COUNT(*), AVG(importance), SUM(access_count) FROM memories")
        
        count, avg_importance, total_accesses = cursor.fetchone()
    ...
    def configure_sync(self, provider: str, bucket: str, auto_sync: bool = False):
        """Configure cloud sync (placeholder)"""
        # Would implement S3/R2/D1 sync in production
        pass
    ...
    def _ensure_section(self, section: str):
        ...
        cursor.execute("SELECT id FROM memory_sections WHERE name = ?", (section,))
        if not cursor.fetchone():
            cursor.execute("""
                INSERT INTO memory_sections (id, name, created_at)
                VALUES (?, ?, ?)
    ...
    def _auto_link_memory(self, memory_id: str):
        """Automatically create links to related memories"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        
        # Get the new memory
        cursor.execute("SELECT content, embedding FROM memories WHERE id = ?", (memory_id,))
        row = cursor.fetchone()
        ...
        cursor.execute("SELECT id, embedding FROM memories WHERE id != ? AND embedding IS NOT NULL", (memory_id,))
        
        for other_id, other_embedding_json in cursor.fetchall():
            other_embedding = json.loads(other_embedding_json)
            similarity = self._cosine_similarity(embedding, other_embedding)
            
            # Create link if similarity > 0.7
            if similarity > 0.7:
