...
# Add vibe-coder path for core.models and core.skill_index
_vibe_coder_dir = Path(__file__).resolve().parent.parent / "engine" / "vibe-coder"
if _vibe_coder_dir.exists() and str(_vibe_coder_dir) not in sys.path:
    sys.path.insert(0, str(_vibe_coder_dir))

try:
    from core.models import Finding
    from core.skill_index import SkillDoc, index_skills
except ImportError:
    # Fallback: define minimal stubs if vibe-coder core is unavailable
    class Finding:
    ...
    def index_skills(repo_root, enforce_lock=False):
        return [], []
...
def build_triggers(*, stacks: Sequence[str], findings: Sequence[Finding], gates: Sequence[Dict[str, Any]], task: str = "") -> List[str]:
    ...
    # discovery mode (no signals) – helpful when users want inspiration / browsing
