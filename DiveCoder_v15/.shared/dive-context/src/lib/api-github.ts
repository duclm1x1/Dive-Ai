 ...
 */
export async function fetchSkillDocs(libraryId: string): Promise<SkillDocs> {
  ...
  throw new Error(
    `Library "${libraryId}" not found in popular libraries. Use GitHub search or add custom repo.`
  );
}
 ...
 */
export async function searchSkills(query: string): Promise<Array<{ name: string; description: string; stars: number }>> {
