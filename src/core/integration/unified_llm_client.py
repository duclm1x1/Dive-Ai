...
class UnifiedLLMClient:
    ...
    def chat_with_premium_model(self, model_name: str, message: str, max_tokens: int = 2000,
                                   provider: Optional[Provider] = None) -> APIResponse:
        """Send message to premium models for code review"""
        ...
        if provider is None:
            ...
            for prov in [Provider.V98API, Provider.AICODING]:
                try:
                    resp = self._send_premium_message(actual_model, message, max_tokens, prov)
                    responses.append(resp)
                except:
                    pass
            ...
            return self._send_premium_message(actual_model, message, max_tokens, provider)
    
    def _send_premium_message(self, model_name: str, message: str, max_tokens: int,
                             provider: Provider) -> APIResponse:
        """Internal method to send message to premium models"""
        try:
            start_time = datetime.now()
