...
def _topic_triggers_from_text(text: str, limit: int = 10) -> List[str]:
    # Pull tokens, bias to "Use when:" area if present
    t = text
    idx = text.lower().find("use when")
    if idx != -1:
        t = text[idx : idx + 600]
    toks = [m.group(0).lower() for m in _TOKEN_RE.finditer(t)]
    # de-noise
    blacklist = {
        "when",
        "use",
    ...
    for tok in toks:
        ...
        if tok in seen:
            continue
        seen.add(tok)
        out.append(f"topic:{tok}")
        if len(out) >= limit:
            break
    return out


def _skill_id_for_file(p: Path, text: str) -> str:
    fm = _parse_frontmatter(text)
    # Prefer explicit id, then name
    for k in ("id", "skill_id", "name"):
        v = str(fm.get(k) or "").strip()
        if v:
            return v
    # Fallback to folder name for SKILL.md packs
...
def generate(repo_root: str) -> Tuple[Dict[str, Any], str]:
    ...
    if external_dir.exists():
        # external packs: only index SKILL.md by default (avoid README noise)
        skill_files += sorted(external_dir.rglob("SKILL.md"))
        skill_files += sorted(external_dir.rglob("skill.md"))
    ...
    for p in skill_files:
        ...
        text = _read(p)
        skill_id = _skill_id_for_file(p, text)

        # Ensure stable uniqueness: for external packs, folder name is a safe disambiguator.
        if skill_id in seen:
        ...
        if skill_id.startswith("vibe-"):
            base_score = 80
        if skill_id in {
            "vibe-super-coder-skills",
            "vibe-qa-debugging",
            "vibe-documentation-auto",
        ...
        # External skills: add topic triggers (keyword routing)
        if p.name.lower() == "skill.md":
            triggers += _topic_triggers_from_text(text)
    ...
    if dup_repos:
        ...
        for r, ids in sorted(dup_repos.items(), key=lambda x: -len(x[1])):
            lines.append(f"- {r}: {', '.join(ids[:15])}{'…' if len(ids)>15 else ''}\n")
        lines.append("\n")
    else:
    ...
    for s in sorted(skills, key=lambda x: (x["kind"], x["skill_id"])):
        trig = ", ".join([t for t in s["triggers"] if t.startswith(("stack:", "category:", "need:", "kind:", "topic:"))][:8])
        if len([t for t in s["triggers"] if t.startswith(("stack:", "category:", "need:", "kind:", "topic:"))]) > 8:
            trig += "…"
