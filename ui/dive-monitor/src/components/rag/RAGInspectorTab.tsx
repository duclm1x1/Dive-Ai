...
import { mockRAGQuery, mockRAGChunks } from '@/data/mockData';
...
export function RAGInspectorTab() {
  ...
  const totalChars = mockRAGChunks.reduce((sum, c) => sum + c.content.length, 0);
      ...
      <div className="w-80 shrink-0 border-r border-border overflow-y-auto bg-card/30 p-4">
        ...
        <div className="mb-4">
          ...
          <div className="mt-1 p-3 rounded-lg bg-background border border-border">
            <p className="text-sm text-foreground">{mockRAGQuery.original}</p>
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="mt-1 space-y-2">
            {mockRAGQuery.enhanced.map((query, index) => (
              <div 
                key={index}
        ...
        {mockRAGQuery.step_back && (
          <div>
            ...
            <div className="mt-1 p-3 rounded-lg bg-accent/5 border border-accent/20">
              <p className="text-sm text-foreground/80">{mockRAGQuery.step_back}</p>
            </div>
          </div>
        )}
