# Dive Coder v15 – Enterprise RAG SOTA (offline-first) + Clawdbot plugin bundle

Drop-in add-on để dùng Vibe Coder như **encapsulated logic OS** cho Antigravity.

## Dive Monitor (UI) + Monitor Server (Option A)

V15 bundle gồm **Dive Coder** (engine) + **Dive Monitor** (UI) + `monitor_server` (FastAPI observability).

Chạy 1 service (serve UI dist-lite + API + SSE):
...
Mở UI: `http://localhost:8787/` (Dive Monitor)

Antigravity ingest events: `POST /v1/ingest` • SSE: `GET /v1/stream/events`
...
## Clawdbot plugin-style bundle (skills pack)

This release includes a **Clawdbot plugin bundle** that ships Dive Coder skills as plugin-provided skills.

Install (local zip):

```bash
clawdbot plugins install ./clawdbot_plugin_dive_coder_v14_4.zip
...
See `antigravity_plugin/docs/ANTIGRAVITY.md` for MCP setup and UI panel plan.
