...
export function EvidenceClaimsTab() {
  const { evidence, claims, evidencePack } = useRuntimeStore();
  ...
  const handleExport = (type: 'evidence' | 'claims' | 'report') => {
    const data = type === 'evidence' 
      ? evidencePack?.evidence || evidence
      : type === 'claims' 
      ? evidencePack?.claims || claims
      : evidencePack;
    ...
    a.href = url;
    a.download = `${type}_${evidencePack?.pack_id || 'export'}.json`;
    a.click();
        ...
        <div className="space-y-2">
          {evidence.length === 0 ? (
            <div className="p-8 text-center text-muted-foreground">
              ...
              <p className="text-xs mt-2">Evidence will appear when DiveCoder collects data</p>
            </div>
          ) : (
            evidence.map((item) => (
              <EvidenceItem
        ...
        <div className="space-y-3">
          {claims.length === 0 ? (
            <div className="p-8 text-center text-muted-foreground">
              ...
              <p className="text-xs mt-2">Claims will appear when DiveCoder analyzes evidence</p>
            </div>
          ) : (
            claims.map((claim) => (
              <ClaimRow 
